...
{
    int sockfd, new_fd;  // listen on sock_fd, new connection on connfd
    char recvBuff[1025];
    ...
    printf("We have made it to the loop\n");
    //Main loop
    while (ros::ok()) {
        sin_size = sizeof their_addr;
        new_fd = accept(sockfd, (struct sockaddr *)&their_addr, &sin_size);
		...
		if (send(new_fd, "Hello, world!", 13, 0) == -1) perror("send");
 		if((nmb = recv(new_fd, recvBuff, 1025, 0)) == -1){
			perror("recv");
			exit(1);
		}
		printf("Server gor answer: '%s'\n",recvBuff);
